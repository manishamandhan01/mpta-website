    ...
    const handlePasteSubmit = () => {
        ...
        const newRows: TradeRow[] = lines.map((line, index) => {
            ...
            return {
                ...
                volume: parseInt(values[4]) || 0,
                over_write_fees: parseFloat(values[5]) || 0,
                fees: parseFloat(values[6]) || 0,
                netAmount: parseFloat(values[7]) || 0,
                avePrice: parseFloat(values[8]) || 0,
                profit: parseFloat(values[9]) || 0,
                percentProfit: values[10] || '',
                days: parseInt(values[11]) || 0,
                rmul: values[12] || '',
                equity: parseFloat(values[13]) || 0,
                setup: values[14] || '',
                reason: values[15] || '',
                notes: values[16] || ''
            };
